...
interface HomeworkModalProps {
  ...
  studentName: string;
  onClose: () => void;
  onAssign: (selectedExercises: { title: string; type: ExerciseType }[], dueDate: string, instructions: string) => void;
  loading: boolean;
}

const HomeworkModal: React.FC<HomeworkModalProps> = ({ isOpen, studentName, onClose, onAssign, loading }) => {
  const [selectedTasks, setSelectedTasks] = useState<string[]>([]); // Array of "Type:Title" strings
  ...
  const handleSubmit = () => {
    ...
    onAssign(payload, dueDate, instructions);
  };
